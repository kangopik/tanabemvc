...
Namespace TANABE_MVC.Models
    Public Class TargetClass
        Public Function GetDataTarget()
            ...
            Dim model = New List(Of TargetModel)()

            Try
                sSQL = "select * from v_master_target_aso"
                ...
                While rd.Read()
                    Dim data = New TargetModel()

                    ' data.prd_aso_id = rd("prd_aso_id")
                    data.target_id = rd("target_id")
                    data.target_year = rd("target_year")
                    data.target_month = rd("target_month")
                    data.target_sbo = rd("target_sbo")
                    data.target_product_code = rd("target_product_code")
                    data.prd_aso_desc = rd("prd_aso_desc")
                    data.target_description = rd("target_description")
                    data.prd_aso_category = rd("prd_aso_category")
                    data.prd_aso_price = rd("prd_aso_price")
                    data.target_plan_qty = rd("target_plan_qty")
                    data.target_plan_value = rd("target_plan_value")
                    data.rep_name = rd("rep_name")
                    data.rep_bo = rd("rep_bo")
                    data.nama_am = rd("nama_am")
                    data.nama_rm = rd("nama_rm")
                    data.rep_region = rd("rep_region")
