...
Public Class ProductController
    ...
    Function Index() As ActionResult
        ...
        Else
            Return View("~/Views/Admin/Product/Product.vbhtml")
        End If
    End Function
    ...
    Function DataViewPartial(ByVal act As String) As ActionResult
        If act <> "" Then
            ...
            Return RedirectToAction("exportto", "product", New With {.export_type = params(1)})
        End If
        Dim model
        Try
        ...
        Return PartialView("~/Views/Admin/Product/DataViewPartial.vbhtml", model)
    End Function
    <HttpPost(), ValidateInput(False)> _
    Function NewProduct(<ModelBinder(GetType(DevExpressEditorsBinder))> ByVal product As ProductModel, ByVal collection As FormCollection) As ActionResult
    ...
    <HttpPost(), ValidateInput(False)> _
    Function DeleteProduct(ByVal collection As FormCollection) As ActionResult
    ...
    Public Function ExportTo(ByVal export_type As String) As ActionResult
        ...
        Select Case export_type.ToUpper()
            Case "CSV"
                Return GridViewExtension.ExportToCsv(ProductGridViewHelper.ExportGridViewSettings, model)
            Case "PDF"
                Return GridViewExtension.ExportToPdf(ProductGridViewHelper.ExportGridViewSettings, model)
            Case "RTF"
                Return GridViewExtension.ExportToRtf(ProductGridViewHelper.ExportGridViewSettings, model)
            Case "XLS"
                Return GridViewExtension.ExportToXls(ProductGridViewHelper.ExportGridViewSettings, model)
            Case "XLSX"
                Return GridViewExtension.ExportToXlsx(ProductGridViewHelper.ExportGridViewSettings, model)
            Case Else
                Return RedirectToAction("DataViewPartial")
        End Select
    End Function
End Class
Public NotInheritable Class ProductGridViewHelper
    ...
    Private Shared Function CreateExportGridViewSettings() As GridViewSettings
        ...
        'settings.SettingsExport.Styles.Cell.HorizontalAlign = HorizontalAlign.Center
