...
Public Class VerVisitRealController
    Inherits Controller
    Dim repo As New r_VerVisitReal
    ...
    Function Index() As ActionResult
        ...
        Else
            ...
            Return View("~/Views/Verification/Realization/Verification.vbhtml")
        End If
    ...
    Public Function DataViewPartial() As ActionResult
        ...
        Try
            ...
            model = repo.GetVisitRealReady(rep_id, Session("sess_month"), Session("sess_year"))
        Catch generatedExceptionName As Exception
            Throw
        End Try
        Return PartialView("~/Views/Verification/Realization/VerVisitRealDataViewPartial.vbhtml", model)
    End Function
    ...
    Public Function GridLookupPartial() As ActionResult
        ...
        Catch generatedExceptionName As Exception
            Throw
        End Try
        Return PartialView("~/Views/Verification/Realization/GridLookupPartial.vbhtml", model)
    End Function
    ...
    Public Function ExportTo(ByVal export_type As String) As ActionResult
        ...
        Select Case export_type.ToUpper()
            Case "CSV"
                Return GridViewExtension.ExportToCsv(VerVisitRealGridViewHelper.ExportGridViewSettings, model)
            Case "PDF"
                Return GridViewExtension.ExportToPdf(VerVisitRealGridViewHelper.ExportGridViewSettings, model)
            Case "RTF"
                Return GridViewExtension.ExportToRtf(VerVisitRealGridViewHelper.ExportGridViewSettings, model)
            Case "XLS"
                Return GridViewExtension.ExportToXls(VerVisitRealGridViewHelper.ExportGridViewSettings, model)
            Case "XLSX"
                Return GridViewExtension.ExportToXlsx(VerVisitRealGridViewHelper.ExportGridViewSettings, model)
            Case Else
    ...
    Public Function MasterDetailPartial(ByVal visit_id As String) As ActionResult
        ...
        Return PartialView("~/Views/Verification/Realization/MasterDetailPartial.vbhtml", model)

    End Function
End Class

Public NotInheritable Class VerVisitRealGridViewHelper
    ...
    Private Shared Function CreateExportGridViewSettings() As GridViewSettings
        ...
        settings.Name = "VER_VISIT_REAL"
        settings.CallbackRouteValues = New With {Key .Controller = "VerVisitReal", Key .Action = "Index"}
        'settings.SettingsExport.Landscape = True
        settings.SettingsExport.ReportHeader = "PT. TANABE INDONESIA"
        settings.SettingsExport.PaperKind = PaperKind.A4
        settings.SettingsExport.FileName = "ver_visit_real_" & _user_name & "_" & Now.ToString("ddMMyyyy HH:mm:ss")
        ...
        settings.SettingsText.Title = "VERIFICATION VISIT REAL"
