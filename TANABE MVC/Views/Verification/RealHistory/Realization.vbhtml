...
<script type="text/javascript">
    ...
    function UnVerifyAllData(s, e) {
        //VerifyPopupControl.Show();
        var visit_id = v_arr_visit_id;
        var param = 'unverify;' + null + ';' + null + ';' + null + ';' + visit_id;
        if (confirm("Are you sure want to unverification selected realizations ?") == true) {
    ...
    function Unverify_Grid(s, e) {
        var visit_id = s.GetMainElement().getAttribute("v_visit_id");
        var param = 'unverify_by_one;' + null + ';' + null + ';' + null + ';' + visit_id;
        if (confirm("Are you sure want to unverifation this realization visit with code " + visit_id + " ?") == true) {
            DataView1.PerformCallback({ prm: param });
        }
    }
</script>
    ...
    <table class="table">
        ...
        <tr>
            ...
            <td class="cell-newlan cell-backround distance-left">
                ...
                @Html.Action("GridLookupPartial", "VerRealHistory")
            ...
            <td class="cell-blank"></td>
                ...
                @Html.DevExpress().Button(Sub(settings)
                                              settings.Name = "btnunverify"
                                              settings.ControlStyle.CssClass = "button"
                                              settings.Text = "Verify"
                                              settings.Images.Image.Url = "~/Content/Images/retrieve.png"
                                              settings.ToolTip = "UnVerify selected row(s)"
                                              settings.ClientSideEvents.Click = "UnVerifyAllData"
                                              settings.Height = 27
                                              settings.ControlStyle.Font.Size = 7
        ...
        <tr>
            @Html.Action("DataViewPartial", "VerRealHistory")
...
<script type="text/javascript">
    ...
    function OnEndCallback(s, e) {
        ...
        if (s.cpUnVerifyFeedback != 'undefined') {
            if (s.cpUnVerifyFeedback == 'success_unverify') {
                alert('Visit realization have been unverified');
                s.cpUnVerifyFeedback = "undefined";
            } else if (s.cpUnVerifyFeedback == 'error_unverify') {
                alert('Visit realization unverification failed');
                s.cpUnVerifyFeedback = "undefined";
            } else if (s.cpUnVerifyFeedback == 'no_record') {
                ...
                btnselectpage.SetEnabled(false);
                s.cpUnVerifyFeedback = "undefined";
            } else if (s.cpUnVerifyFeedback == 'record_exists') {
                btnunselectall.SetEnabled(false);
                btnunverify.SetEnabled(false);
                btnselectallrow.SetEnabled(true);
                btnselectpage.SetEnabled(true);
