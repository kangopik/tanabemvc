...
<script type="text/javascript">
    ...
    function do_selectAll(s, e) {
        gridSalesPlanVerification.SelectRows();
        btnVerify.SetEnabled(true);
        btnUnselect.SetEnabled(true);
    }

    function do_selectPage(s,e) {
        gridSalesPlanVerification.SelectAllRowsOnPage();
        btnVerify.SetEnabled(true);
        btnUnselect.SetEnabled(true);
    }

    function do_unselect(s,e) {
        gridSalesPlanVerification.UnselectRows();
        btnVerify.SetEnabled(false);
        btnUnselect.SetEnabled(false); 
    }

    function gridSalesPlanVerification_EndCallBack(s, e) {
    ...
    function SelectionChanged(s, e) {
        var i = gridSalesPlanVerification.GetSelectedRowCount();
        if (i == 0) {
            btnVerify.SetEnabled(false);
            btnUnselect.SetEnabled(false);
        } else {
...
<div class="panel-content">
    <table class="table">
        <tr>
            <td class="title-form" style="text-align: center; font-weight: bold;" colspan="11">:: SALES PLAN VERIFICATION ::</td>
        </tr>
        <tr>
            <td class="cell-month cell-backround">
                @Html.DevExpress().ComboBox(Sub(settings)
                                            ...
                                            End Sub).BindList(TANABE_MVC.SalesPlanVerificationController.dsRep).GetHtml()
            </td>
            <td class="cell-month cell-backround">
                @Html.DevExpress().ComboBox(Sub(settings)
            ...
            <td class="cell-blank"></td>
            <td class="cell-reset cell-backround">
                @Using (Html.BeginForm("ExportTo", "SalesPlanVerification"))
                    @Html.DevExpress().Button(Sub(settings)
                                                  settings.Name = "PDF"
                                                  settings.ControlStyle.CssClass = "button"
        ...
        <tr>
            @Html.Action("ViewSalesPlanVerification", "SalesPlanVerification")
        </tr>
    </table>
</div>