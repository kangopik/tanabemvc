...
<script type="text/javascript">
    ...
    function SelectionChanged(s, e) {
        var i = gridSalesPlanHistoryVerification.GetSelectedRowCount();
        if (i == 0) {
            ...
            gridSalesPlanHistoryVerification.GetSelectedFieldValues("sales_id", GetSelectedFieldValuesCallback);
        }
    }

    function gridSalesPlanHistoryVerification_EndCallBack(s, e) {
        if (s.cpInsertResult == "success_unverify") {
            alert("Un Verification has been successfuly proceed");
            s.cpInsertResult = "undefined";
        } else if (s.cpInsertResult == "error_unverify") {
            alert("There is an error on un verification process with specified id " + s.cpErrorCode);
            s.cpInsertResult = "undefined";
        ...
        if (rep_id != '') {
            if (month != null) {
                if (year != null) {
                    ...
                    gridSalesPlanHistoryVerification.PerformCallback({ prm: param });
    ...
    function do_selectAll(s, e) {
        gridSalesPlanHistoryVerification.SelectRows();
        btnVerify.SetEnabled(true);
        btnUnselect.SetEnabled(true);
    }

    function do_selectPage(s, e) {
        gridSalesPlanHistoryVerification.SelectAllRowsOnPage();
        btnVerify.SetEnabled(true);
        btnUnselect.SetEnabled(true);
    }

    function do_unselect(s, e) {
        gridSalesPlanHistoryVerification.UnselectRows();
        btnVerify.SetEnabled(false);
        btnUnselect.SetEnabled(false);
    }
    ...
    function verify_Grid(s, e) {
        ...
        if (confirm("Are you sure want to un-verify this plan with code " + sales_id + " ?") == true) {
            gridSalesPlanHistoryVerification.PerformCallback({ prm: param });
        }
    }
...
<div class="panel-content">
    <table class="table">
        <tr>
            <td class="title-form" style="text-align: center; font-weight: bold;" colspan="11">:: SALES PLAN VERIFICATION HISTORY ::</td>
        </tr>
        <tr>
            ...
            <td class="cell-blank"></td>
            <td class="cell-reset cell-backround">
                @Using (Html.BeginForm("ExportTo", "SalesPlanHistoryVerification"))
                    @Html.DevExpress().Button(Sub(settings)
                                                  settings.Name = "PDF"
                                                  settings.ControlStyle.CssClass = "button"
        ...
        <tr>
            ...
            <td class="cell-reset cell-backround">
                @Html.DevExpress().Button(Sub(settings)
                                              ...
                                              settings.ControlStyle.CssClass = "button"
                                              settings.Text = "UnVerify Selected Records"
                                              settings.ToolTip = "UnVerify Selected Records"
                                              settings.UseSubmitBehavior = True
        ...
        <tr>
            @Html.Action("ViewSalesPlanHistory", "SalesPlanHistoryVerification")
        </tr>
    </table>
</div>