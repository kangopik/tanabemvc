...
<script type="text/javascript">
    ...
    function gridSalesRealHistoryVerification_EndCallBack(s, e) {
        if (s.cpInsertResult == "success_unverify") {
            alert("UnVerification has been successfuly proceed");
            s.cpInsertResult = "undefined";
        } else if (s.cpInsertResult == "error_unverify") {
            alert("There is an error on unverification process with specified id " + s.cpErrorCode);
            s.cpInsertResult = "undefined";
    ...
    function SelectionChanged(s, e) {
        var i = gridSalesRealHistoryVerification.GetSelectedRowCount();
        if (i == 0) {
    ...
    function do_selectAll(s, e) {
        gridSalesRealHistoryVerification.SelectRows();
        btnVerify.SetEnabled(true);
        btnUnselect.SetEnabled(true);
    }

    function do_selectPage(s, e) {
        gridSalesRealHistoryVerification.SelectAllRowsOnPage();
        btnVerify.SetEnabled(true);
        btnUnselect.SetEnabled(true);
    }

    function do_unselect(s, e) {
        gridSalesRealHistoryVerification.UnselectRows();
        btnVerify.SetEnabled(false);
        btnUnselect.SetEnabled(false);
    }
...
<div class="panel-content">
    <table class="table">
        <tr>
            <td class="title-form" style="text-align: center; font-weight: bold;" colspan="11">:: SALES REAL VERIFICATION HISTORY ::</td>
        </tr>
        <tr>
            ...
            <td class="cell-blank"></td>
            <td class="cell-reset cell-backround">
                @Using (Html.BeginForm("ExportTo", "SalesRealHistoryVerification"))
                    @Html.DevExpress().Button(Sub(settings)
                                                  settings.Name = "PDF"
                                                  settings.ControlStyle.CssClass = "button"
        ...
        <tr>
            @Html.Action("ViewSalesRealHistory", "SalesRealHistoryVerification")
        </tr>
    </table>
</div>