...
Namespace Repositories
    Public Class ISalesPlanVerificationHistory
        ...
        Public Function ds_sales(ByVal rep_id_selection As String, ByVal sess_month As Integer, ByVal sess_year As Integer) As List(Of v_sales_planModel)
            Try
                Dim param As DynamicParameters = New DynamicParameters()
                ...
                connection()
                con.Open()
                Dim tb = con.Query(Of v_sales_planModel)("SELECT * FROM [v_sales_plan] WHERE rep_id = @rep_id AND [sales_date_plan] = @sales_date_plan AND [sales_year_plan] = @sales_year_plan AND sales_plan_verification_status = 1 AND sales_realization = 0 ORDER BY dr_code ASC", param, commandType:=CommandType.Text)
                Return tb.ToList()
            Catch ex As Exception
                Throw ex
            Finally
        ...
        Public Function UnverificationSalesPlan(ByVal sales_id As String, ByVal mth As Integer) As Boolean
            Try
                Dim param As DynamicParameters = New DynamicParameters()
                param.Add("@sales_id", sales_id)
                param.Add("@month", mth)
                connection()
                con.Open()
                con.Execute("SP_UNVERIFICATION_SALES_PLAN", param, commandType:=CommandType.StoredProcedure)
                Return True
            Catch ex As Exception
                Throw ex
                Return False
        ...
        Public Function UnverificationOneByOne(ByVal sales_id As String, ByVal mth As Integer) As Boolean
            Try
                Dim param As DynamicParameters = New DynamicParameters()
                param.Add("@sales_id", sales_id)
                param.Add("@month", mth)
