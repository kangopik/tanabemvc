...
Namespace Repositories
    Public Class ISalesActual
        ...
        Public Function dsProductLookup(ByVal rep_id As String) As List(Of m_productModel)
            Try
                Dim param As DynamicParameters = New DynamicParameters()
                param.Add("@rep_id", rep_id)
                connection()
                con.Open()
                Dim tb = con.Query(Of m_productModel)("SELECT * FROM [m_product] WHERE prd_status = 1", param, commandType:=CommandType.Text)
                Return tb.ToList()
            Catch ex As Exception
                Throw ex
            Finally
        ...
        Public Function dsProductSales(ByVal rep_id As String, ByVal sess_month As Integer, ByVal sess_year As Integer) As List(Of v_sales_productModel)
            Try
                ...
                connection()
                con.Open()
                Dim tb = con.Query(Of v_sales_productModel)("SELECT * FROM [v_sales_product] WHERE rep_id = @rep_id AND [sales_date_plan] = @sales_date_plan AND [sales_year_plan] = @sales_year_plan AND [sales_plan_verification_status] = 1 AND sales_realization = 1 ORDER BY dr_code ASC", param, commandType:=CommandType.Text)
                Return tb.ToList()
            Catch ex As Exception
                Throw ex
            Finally
