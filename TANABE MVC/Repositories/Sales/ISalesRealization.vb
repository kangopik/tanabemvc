...
Namespace Repositories
    Public Class ISalesRealization
        ...
        Public Function dsProductSales(ByVal rep_id_selection As String, ByVal sess_month As Integer, ByVal sess_year As Integer) As List(Of v_sales_productModel)
            Try
                ...
                connection()
                con.Open()
                Dim tb = con.Query(Of v_sales_productModel)("SELECT * FROM [v_sales_product] WHERE rep_id = @rep_id AND [sales_date_plan] = @sales_date_plan AND [sales_year_plan] = @sales_year_plan AND [sales_plan_verification_status] = 1 ORDER BY dr_code ASC", param, commandType:=CommandType.Text)
                Return tb.ToList()
            Catch ex As Exception
                Throw ex
            Finally
