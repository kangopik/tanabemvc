...
Namespace Repositories
    Public Class ISalesRealVerficationHistory
        ...
        Public Function dsProductSales(ByVal rep_id_selection As String, ByVal sess_month As Integer, ByVal sess_year As Integer) As List(Of v_sales_productModel)
            Try
                Dim param As DynamicParameters = New DynamicParameters()
                ...
                connection()
                con.Open()
                Dim tb = con.Query(Of v_sales_productModel)("SELECT * FROM [v_sales_product] WHERE rep_id = @rep_id AND [sales_date_plan] = @sales_date_plan AND [sales_year_plan] = @sales_year_plan AND [sales_plan_verification_status] = 1 AND sales_realization = 1 AND [sales_real_verification_status] = 1 ORDER BY dr_code ASC", param, commandType:=CommandType.Text)
                Return tb.ToList()
            Catch ex As Exception
                Throw ex
            Finally
        ...
        Public Function Unverify(ByVal sales_id As String) As Boolean
            Try
                Dim param As DynamicParameters = New DynamicParameters()
                param.Add("@sales_id", sales_id)
                param.Add("@sales_real_verification_by", "")
                param.Add("@sales_real_verification_date", Date.Now())
                connection()
                con.Open()
                con.Execute("UPDATE t_sales SET sales_real_verification_status= 0, sales_real_verification_by = @sales_real_verification_by, sales_real_verification_date = @sales_real_verification_date WHERE sales_id=@sales_id", param, commandType:=CommandType.Text)
                Return True
            Catch ex As Exception
                Throw ex
                Return False
        ...
        Public Function UnverifyOneByOne(ByVal sales_id As String, ByVal sales_real_verification_by As String, ByVal sales_real_verification_date As DateTime) As Boolean
            Try
                Dim param As DynamicParameters = New DynamicParameters()
                param.Add("@sales_id", sales_id)
                param.Add("@sales_real_verification_by", sales_real_verification_by)
                param.Add("@sales_real_verification_date", sales_real_verification_date)
                connection()
                con.Open()
                con.Execute("UPDATE t_sales SET sales_real_verification_status= 0, sales_real_verification_by = @sales_real_verification_by, sales_real_verification_date = @sales_real_verification_date WHERE sales_id=@sales_id", param, commandType:=CommandType.Text)
                Return True
