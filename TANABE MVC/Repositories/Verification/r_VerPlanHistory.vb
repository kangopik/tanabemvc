...
Namespace Repositories
    Public Class r_VerPlanHistory
        ...
        Public Function GetVisitPlanVerified(ByVal rep_id As String, ByVal month As String, ByVal year As String) As List(Of VerPlanHistoryModel)
            Try
                ...
                param.Add("@year", year)
                myConnection.Open()
                Dim tb = SqlMapper.Query(Of VerPlanHistoryModel)(myConnection, "SELECT * FROM [v_visit_plan] WHERE rep_id = @rep_id AND (DATEPART(month,visit_date_plan) = @month AND DATEPART(year,visit_date_plan) = @year) " & _
        ...
        Function processUnVerify(ByVal visit_id As String) As Boolean
            ...
            Try
                ...
                For i As Integer = 0 To v_visit_id.Length - 1
                    ...
                    Dim cmd As New SqlCommand("[SP_UNVERIFICATION_VISIT_PLAN]", myConnection)
